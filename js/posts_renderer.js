...
class PostsRenderer {
  ...
  static post2html(post) {
    ...
    let el      = oDOM.documentElement.children[1].children[0]; // root -> body -> articale
  ...
  hash({postId, pageId, tag}) {
    ...
    if (tag) _hash = "#!tag/" + encodeURIComponent(tag);
    if (pageId) {
      if (tag)
        _hash += "/";
      else
  ...
  unhash(_hash = location.hash) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      case "tag":
        i++;
        if (i < data.length) ctxt.tag = decodeURIComponent(data[i]);
        break;
      case "post":
        i++;
        if (i < data.length) try {
  ...
  update() {
    ...
    return (ctxt.archive ? this.loadArchive() : this.loadPosts(ctxt)).then(function() {
      window.scrollTo(0, 0);
  ...
  loadPosts(ctxt) {
    ...
    return this.posts_loader.getPosts(ctxt).then(
        posts => {
          this.clean();
          posts.forEach(post => this.container.appendChild(PostsRenderer.post2html(post)));
          prettyPrint();
          if (this.posts_loader.hasPrev()) {
            this.prevBtn.style.display = "inline";
            this.prevBtn.href          = this.hash(this.posts_loader.prev);
          }
          if (this.posts_loader.hasNext()) {
            this.nextBtn.style.display = "inline";
            this.nextBtn.href          = this.hash(this.posts_loader.next);
          }
          if (posts.length === 0) {
            this.container.innerHTML = "<p>Oops! 404 Error!</p>";
            return Promise.reject();
          }
          return Promise.resolve();
        },
        err => {
          this.container.innerHTML = "<p>Oops! Error loading posts!</p>";
          console.error(err);
          return Promise.reject();
        });
  }
